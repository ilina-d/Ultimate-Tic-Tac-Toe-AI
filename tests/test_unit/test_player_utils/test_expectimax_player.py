...
from utils.players.expectimax_player import ExpectiMaxPlayer


class TestExpectiMaxPlayer:
    """ Class to test the functionality of the ExpectiMaxPlayer class. """
    ...
    @pytest.mark.parametrize(
        "is_maximizing, moves_made, use_randomness, prev_small_idx, board_empty, expected_result, error_msg", (
    ...
    def test_get_premove(self, is_maximizing, moves_made, use_randomness, prev_small_idx, board_empty, expected_result,
        ...
        player = ExpectiMaxPlayer(use_randomness=use_randomness)
    ...
    # BCC criteria:
    # A: use_timed_depth value
    #   1 - True, 2 - False
    # B: use_dynamic_depth value
    #   1 - True, 2 - False
    ...
    def test_update_target_depth(self, use_timed, use_dynamic, moves_made, initial_depth, expected_depth_change,
        ...
        if use_timed:
            player = ExpectiMaxPlayer(target_depth='timed')
        elif use_dynamic:
            player = ExpectiMaxPlayer(target_depth='dynamic')
        else:
            player = ExpectiMaxPlayer(target_depth=initial_depth)
    ...
    @pytest.mark.parametrize(
        "player_sign, moves_made, initial_moves_setup, prev_small_idx, use_premove_scenario, error_msg", (
                ...
                # A1 B2 C2 D2
                ('X', 5, "couple_moves_made", None, False,
                 "Should handle None prev_small_idx and choose from available boards."),
                # A2 B2 C3 D1
    ...
    @patch('utils.players.expectimax_player.StateUpdater')
    @patch('utils.players.expectimax_player.StateChecker')
    def test_make_move(self, mock_checker, mock_updater, player_sign, moves_made, initial_moves_setup,
        ...
        """ Test whether ExpectiMaxPlayer makes legal and optimal moves. """
        ...
        player = ExpectiMaxPlayer(target_depth=2, use_randomness=False)
    ...
    #   1 - True, 2 - False
    # C: is_averaging value
    #   1 - True, 2 - False
    # happy path: A2 B1 C2

    @pytest.mark.parametrize(
        "game_won, at_depth_limit, is_maximizing, is_averaging, player_sign, target_depth, curr_depth, error_msg", (
                # A1 B1 C2 (happy path)
                (True, False, True, False, 'X', 3, 0, "Should return heuristic score when game is won."),
                # A4 B1 C2
                (False, True, True, False, 'X', 3, 3, "Should return heuristic score at depth limit."),
                # A1 B2 C2
                (True, False, False, False, 'O', 3, 0, "Should return heuristic score when game is won for minimizer."),
                # A4 B2 C2
                (False, True, False, False, 'O', 3, 3, "Should return heuristic score at depth limit for minimizer."),
                # A1 B1 C1
                (True, False, True, True, 'X', 3, 0, "Should return heuristic score when game is won, regardless of averaging flag."),
                # A4 B1 C1
                (False, True, True, True, 'X', 3, 3, "Should return heuristic score at depth limit when averaging."),
                # A4 B2 C1
                (False, True, False, True, 'O', 3, 3, "Should return heuristic score at depth limit for minimizer when averaging."),
        ))
    @patch('utils.players.expectimax_player.StateEvaluator')
    @patch('utils.players.expectimax_player.StateUpdater')
    @patch('utils.players.expectimax_player.StateChecker')
    def test_expectimax(self, mock_checker, mock_updater, mock_evaluator, game_won, at_depth_limit,
                        is_maximizing, is_averaging, player_sign, target_depth, curr_depth, error_msg):
        """ Test whether expectimax algorithm works correctly with averaging nodes. """

        state = self.test_get_state("couple_moves_left")
        legal_moves = self.test_get_legal_moves("couple_moves_left")

        mock_checker.check_win.return_value = game_won
        mock_evaluator.heuristic.return_value = 100 if is_maximizing else -100
        mock_updater.update_state.return_value = (state, False)

        player = ExpectiMaxPlayer(target_depth=target_depth)
        player.sign = player_sign
        player.legal_moves = legal_moves

        score = player.expectimax(state, 5, curr_depth, is_maximizing, is_averaging)
