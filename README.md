...
git clone https://github.com/ilina-d/Ultimate-Tic-Tac-Toe-AI.git
...
#### Original UI With Expanded Control:
```py
# Important to call the original ui version (defaults to user-friendly):
from utils.game.game_ui import GameUI
...
I also expanded this project to contain a comprehencive testing module - covering unit, integration and UI testing with the help of the pytest and unittest packages. All tests follow standard practices like Base Choice Coverage for most input-space based tests and Prime Path Coverage for graph-based testing, ensuring both coverage of all scenarios with a minimal test set. 

**Helper classes** are introduced for easy setup of tests that use the complex state object (check out the [state_generator](tests/state_generator.py) and [sample_generator](tests/sample_generator.py)), and the [base_ui_test](tests/test_ui/ui_test_utils.py) class (from which all ui tests inherit) for handling the need for parallelization while executing the UI and its tests.
